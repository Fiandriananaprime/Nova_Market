...
export default function BuyerLayout() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 h-16">
            <Link to="/shop" className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-bold text-lg font-display text-[var(--foreground)] hidden sm:block">MasoMarket</span>
            </Link>
            ...
            <form className="flex-1 max-w-2xl" onSubmit={(e) => { e.preventDefault(); navigate(`/products?q=${search}`); }}>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 text-sm bg-[var(--secondary)] border border-[var(--border)] rounded-xl text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-2 focus:ring-[#0077B6]/25 focus:border-[#0077B6] transition-all"
                />
              </div>
            </form>
            ...
            <div className="flex items-center gap-0.5 ml-auto">
              ...
              <button
                ...
                className="hidden sm:flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-[var(--muted-foreground)] hover:bg-[var(--secondary)] rounded-lg transition-colors"
              ...
              <button
                ...
                className="p-2 text-[var(--muted-foreground)] hover:bg-[var(--secondary)] rounded-lg transition-colors"
              ...
              <Link
                to="/favorites"
                className="relative p-2 text-[var(--muted-foreground)] hover:bg-[var(--secondary)] rounded-lg transition-colors"
              ...
              {/* Notifications */}
              <Link to="/orders" className="hidden sm:block relative p-2 text-[var(--muted-foreground)] hover:bg-[var(--secondary)] rounded-lg transition-colors" title="Notifications">
              ...
              <div className="relative" ref={cartRef}>
                ...
                {cartOpen && (
                  <div className="absolute right-0 top-full mt-2 w-80 bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-2xl z-50 overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
                      <span className="font-semibold text-sm font-display text-[var(--foreground)]">{t('Cart', 'Panier')} ({cartCount})</span>
                      <button onClick={() => setCartOpen(false)} className="text-[var(--muted-foreground)] hover:text-[var(--foreground)]"><X className="w-4 h-4" /></button>
                    </div>
                    {cart.length === 0 ? (
                      ...
                      <>
                          ...
                          {cart.map(item => (
                            <div key={item.productId} className="flex items-center gap-3 px-4 py-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-[var(--muted-foreground)]">x{item.qty} · {formatPrice(item.price)}</div>
                              </div>
                              <button onClick={() => removeFromCart(item.productId)} className="text-[var(--muted-foreground)] hover:text-red-500 transition-colors flex-shrink-0">
                        ...
                        <div className="px-4 py-3 border-t border-[var(--border)] bg-[var(--secondary)]">
                          <div className="flex items-center justify-between text-sm mb-3">
                            ...
                            <span className="font-bold text-[var(--foreground)]">{formatPrice(cartTotal)}</span>
                          </div>
                          <div className="flex gap-2">
                            <button
                              onClick={() => { navigate('/cart'); setCartOpen(false); }}
                              className="flex-1 py-2 text-sm font-medium border border-[var(--border)] rounded-xl text-[var(--foreground)] hover:bg-[var(--border)] transition-colors"
                            >
              ...
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className="flex items-center gap-1.5 pl-1 pr-2 py-1 rounded-xl hover:bg-[var(--secondary)] transition-colors"
                >
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-1.5 w-52 bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-2xl z-50">
                    <div className="p-2">
                      <div className="flex items-center gap-2.5 px-3 py-2.5 mb-1">
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-sm text-[var(--foreground)] truncate">Andry Rakoto</div>
                          <div className="text-xs text-[var(--muted-foreground)] truncate">andry@email.com</div>
                        </div>
                      </div>
                      ...
                      {[
                        ...
                        <Link
                          ...
                          className="flex items-center gap-2.5 px-3 py-2 text-sm text-[var(--foreground)] hover:bg-[var(--secondary)] rounded-xl transition-colors"
                        >
      ...
      {/* Mobile bottom nav */}
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-[var(--card)] border-t border-[var(--border)] z-40 safe-area-pb">
          ...
          {[
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex flex-col items-center justify-center gap-0.5 transition-colors ${active ? 'text-[#0077B6]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'}`}
              >
                {item.icon}

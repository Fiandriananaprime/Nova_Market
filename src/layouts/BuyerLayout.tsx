        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 h-16">
            <Link to="/shop" className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="font-bold text-lg font-display text-secondary-foreground hidden sm:block">MasoMarket</span>
            </Link>
            ...
            <form className="flex-1 max-w-2xl" onSubmit={(e) => { e.preventDefault(); navigate(`/products?q=${search}`); }}>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 text-sm bg-secondary border border-border rounded-xl text-secondary-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary transition-all"
                />
              </div>
            </form>
            ...
            <div className="flex items-center gap-0.5 ml-auto">
              ...
              >
                ...
                {favorites.length > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 text-[10px] bg-accent text-white rounded-full flex items-center justify-center font-bold">
                    {favorites.length}
                  </span>
                )}
              </Link>
              ...
              <div className="relative" ref={cartRef}>
                ...
                >
                  ...
                  {cartCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 w-4 h-4 text-[10px] bg-primary text-white rounded-full flex items-center justify-center font-bold">
                      {cartCount}
                    </span>
                  )}
                </button>
                ...
                {cartOpen && (
                  <div className="absolute right-0 top-full mt-2 w-80 bg-card border border-border rounded-2xl shadow-2xl z-50 overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                      <span className="font-semibold text-sm font-display text-secondary-foreground">{t("Cart")} ({cartCount})</span>
                      <button onClick={() => setCartOpen(false)} className="text-muted-foreground hover:text-secondary-foreground"><X className="w-4 h-4" /></button>
                    </div>
                    {cart.length === 0 ? (
                      ...
                      <>
                          ...
                          {cart.map(item => (
                            <div key={item.productId} className="flex items-center gap-3 px-4 py-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-xs font-medium text-secondary-foreground truncate">{item.name}</div>
                        ...
                        <div className="px-4 py-3 border-t border-border bg-secondary">
                          <div className="flex items-center justify-between text-sm mb-3">
                            <span className="font-medium text-secondary-foreground">Total</span>
                            <span className="font-bold text-secondary-foreground">{formatPrice(cartTotal)}</span>
                          </div>
                          <div className="flex gap-2">
                            <button
                              onClick={() => { navigate('/cart'); setCartOpen(false); }}
                              className="flex-1 py-2 text-sm font-medium border border-border rounded-xl text-secondary-foreground hover:bg-border transition-colors"
                            >
                              {t("View cart")}
                            </button>
                            <button
                              onClick={() => { navigate('/checkout'); setCartOpen(false); }}
                              className="flex-1 py-2 text-sm font-medium bg-primary text-white rounded-xl hover:bg-[#005f92] transition-colors flex items-center justify-center gap-1"
                            >
              ...
              <div className="relative" ref={profileRef}>
                ...
                >
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-xs font-bold">A</div>
                  ...
                  <div className="absolute right-0 top-full mt-1.5 w-52 bg-card border border-border rounded-2xl shadow-2xl z-50">
                    <div className="p-2">
                      <div className="flex items-center gap-2.5 px-3 py-2.5 mb-1">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-sm font-bold flex-shrink-0">A</div>
                        <div className="min-w-0">
                          <div className="font-semibold text-sm text-secondary-foreground truncate">Andry Rakoto</div>
                      ...
                      {[
                        ...
                        <Link
                          ...
                          className="flex items-center gap-2.5 px-3 py-2 text-sm text-secondary-foreground hover:bg-secondary rounded-xl transition-colors"
                        >
          ...
          {[
            ...
            { icon: (
              <div className="relative">
                ...
                {cartCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 text-[9px] bg-primary text-white rounded-full flex items-center justify-center font-bold">{cartCount}</span>}
              </div>
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex flex-col items-center justify-center gap-0.5 transition-colors ${active ? 'text-primary' : 'text-muted-foreground hover:text-secondary-foreground'}`}
              >
                {item.icon}

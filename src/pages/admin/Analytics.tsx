...
import { AnalyticsPeriod, AnalyticsResponse } from "@/type/admin/Analytics";

const dateFilters: { label: string; value: AnalyticsPeriod }[] = [
...
const categoryColors = ["#0077B6", "#5ABCB9", "#8da8b5", "#D0CCD0"];
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        toast(getApiErrorMessage(error, "Error while fetching analytics"), "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading && !data)
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-sm text-muted-foreground">{t("Loading analytics...")}</div>
      </div>
    );

  const revenueSeries = data?.revenueSeries ?? [];
  const topSellers = data?.topSellers ?? [];
  const categoryData = data?.topCategories?.map(({ name, percentage }) => ({
    name,
    value: percentage,
  })) ?? [];
  ...
  return (
    <div className="space-y-5">
      ...
      {period === "custom" && (
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            ].map(({ id, label, value, setValue, min, max }) => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { title: "Revenue", value: formatPrice(data?.totalRevenue ?? 0), icon: <DollarSign className="w-5 h-5" />, color: "#0077B6" },
          { title: "Orders", value: String(data?.totalOrders ?? 0), icon: <ShoppingCart className="w-5 h-5" />, color: "#5ABCB9" },
          { title: "Buyers", value: String(data?.totalBuyers ?? 0), icon: <Users className="w-5 h-5" />, color: "#0077B6" },
          { title: "Sellers", value: String(data?.totalSellers ?? 0), icon: <TrendingUp className="w-5 h-5" />, color: "#5ABCB9" },
        ].map((stat) => (
          <StatCard key={stat.title} {...stat} title={t(stat.title)} />
        ))}
      </div>

      <div className="grid lg:grid-cols-2 gap-5">
        <BarStat title={t("Revenue over time")} data={revenueSeries} />
          ...
          {topSellers.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={topSellers} layout="vertical" barSize={14}>
                ...
                />
                <Tooltip
                  formatter={(value) => [formatPrice(Number(value ?? 0)), t("Revenue")]}
                  contentStyle={{
                    background: "var(--card)",
                    border: "1px solid var(--border)",
                    borderRadius: "8px",
                ...
                />
                <Bar dataKey="revenue" fill="#5ABCB9" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
            ...
            <div className="flex items-center gap-4">
              <ResponsiveContainer width="60%" height={180}>
                <PieChart>
                  ...
                  <Tooltip formatter={(value) => [`${Number(value ?? 0)}%`, t("Sales")]} />
                </PieChart>
              </ResponsiveContainer>

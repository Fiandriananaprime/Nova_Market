          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--secondary)]">
                {['Order ID', 'Buyer', 'Seller', 'Amount', 'Payment', 'Status', 'Date'].map(h => (
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {filtered.map(order => (
                <tr key={order.id} className="hover:bg-[var(--secondary)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-[var(--foreground)]">{order.buyer}</td>
                  <td className="px-4 py-3 text-[var(--foreground)]">{order.seller}</td>
                  <td className="px-4 py-3 font-medium text-[var(--foreground)] whitespace-nowrap">{formatPrice(order.amount)}</td>
                  <td className="px-4 py-3 text-[var(--muted-foreground)]">{order.payment}</td>

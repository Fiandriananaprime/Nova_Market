...
export default function Checkout() {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-8">
      <h1 className="text-2xl font-bold font-display text-secondary-foreground mb-6">{t("Checkout-Commande")}</h1>
        ...
        {steps.map((s, i) => (
          <div key={s.id} className="flex items-center flex-1">
            <div className={`flex flex-col items-center ${i > 0 ? 'flex-1' : ''}`}>
              {i > 0 && <div className={`h-0.5 w-full ${i <= currentIdx ? 'bg-primary' : 'bg-border'} mb-2`} />}
              <div className={`flex items-center gap-2 ${i === 0 ? '' : ''}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i < currentIdx ? 'bg-primary text-white' : i === currentIdx ? 'bg-primary text-white ring-4 ring-primary/20' : 'bg-secondary text-muted-foreground'}`}>
                  {i < currentIdx ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
                </div>
                <span className={`text-xs font-medium hidden sm:block ${i === currentIdx ? 'text-primary' : i < currentIdx ? 'text-secondary-foreground' : 'text-muted-foreground'}`}>{s.label}</span>
              </div>
            </div>
      ...
      <div className="grid md:grid-cols-5 gap-6">
        <div className="md:col-span-3 space-y-4">
          {step === 'delivery' && (
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h2 className="font-semibold font-display text-secondary-foreground">{t("Delivery address")}</h2>
              </div>
          ...
          {step === 'method' && (
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-semibold font-display text-secondary-foreground">{t("Delivery method")}</h2>
              </div>
              <div className="space-y-2 mb-5">
                {[
                  ...
                  <label key={m.id} className={`flex items-center gap-3 p-3.5 border-2 rounded-xl cursor-pointer transition-colors ${deliveryMethod === m.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'}`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${deliveryMethod === m.id ? 'border-primary bg-primary' : 'border-border'}`} />
                    <div className="flex-1">
                      <div className="font-medium text-sm text-secondary-foreground">{m.label}</div>
                      <div className="text-xs text-muted-foreground">{m.desc}</div>
                    </div>
                    <span className="font-semibold text-sm text-secondary-foreground">{m.price === 0 ? t("Free") : formatPrice(m.price)}</span>
                  </label>
                ))}
              </div>
          ...
          {step === 'payment' && (
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-semibold font-display text-secondary-foreground">{t("Payment")}</h2>
              </div>
              <div className="space-y-2 mb-5">
                {[
                  ...
                  <label key={m.id} className={`flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition-colors ${paymentMethod === m.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'}`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${paymentMethod === m.id ? 'border-primary bg-primary' : 'border-border'}`} />
          ...
          {step === 'review' && (
            <div className="bg-card border border-border rounded-xl p-5">
              <h2 className="font-semibold font-display text-secondary-foreground mb-4">{t("Review your order")}</h2>
              <div className="space-y-2 mb-5">
                {cart.map(item => (
                  <div key={item.productId} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-secondary-foreground truncate">{item.name}</div>
                      <div className="text-xs text-muted-foreground">x{item.qty}</div>
                    </div>
                    <span className="text-sm font-semibold text-secondary-foreground">{formatPrice(item.price * item.qty)}</span>
                  </div>
                ))}
              </div>
              <div className="flex gap-2">
        ...
        <div className="md:col-span-2">
          <div className="bg-card border border-border rounded-xl p-4 sticky top-24">
            <h3 className="font-semibold font-display text-secondary-foreground mb-3 text-sm">{t("Summary")}</h3>
            <div className="space-y-1.5 text-sm mb-3">
              ...
              <div className="border-t border-border pt-2 flex justify-between"><span className="font-semibold text-secondary-foreground">Total</span><span className="font-bold text-secondary-foreground">{formatPrice(total)}</span></div>
            </div>
          </div>
        </div>
      </div>

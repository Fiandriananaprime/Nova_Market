...
export default function Favorites() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold font-display text-[var(--foreground)] mb-5">{t('My Favorites', 'Mes Favoris')}</h1>
          ...
          favProducts.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {favProducts.map(product => (
                <div key={product.id} className="bg-[var(--card)] border border-[var(--border)] rounded-xl overflow-hidden group">
                  <div className="relative overflow-hidden bg-[var(--secondary)] cursor-pointer" onClick={() => navigate(`/products/${product.id}`)}>
                  ...
                  <div className="p-3">
                    ...
                    <h3 className="font-medium text-sm text-[var(--foreground)] truncate">{product.name}</h3>

    ...
    <div className="pb-20 sm:pb-6">
      ...
      <div className="mt-4 grid lg:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {images.map((img, i) => (
              <button key={i} onClick={() => setActiveImg(i)} className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${activeImg === i ? 'border-primary' : 'border-border'}`}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <h1 className="text-2xl font-bold font-display text-secondary-foreground mb-2">{product.name}</h1>
            <Rating value={product.rating} count={product.reviews} />
          </div>

          <div className="flex items-baseline gap-3">
            <span className="text-3xl font-bold text-secondary-foreground font-display">{formatPrice(product.price)}</span>
          ...
          <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl cursor-pointer hover:bg-border/50 transition-colors" onClick={() => navigate(`/stores/${seller.id}`)}>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-1.5">
                <span className="font-medium text-sm text-secondary-foreground">{seller.name}</span>
          ...
          {/* Qty */}
          <div>
            <label className="text-sm font-medium text-secondary-foreground mb-2 block">{t("Quantity")}</label>
            <div className="flex items-center gap-3">
              <div className="flex items-center border border-border rounded-xl overflow-hidden">
                ...
                <span className="w-12 text-center font-medium text-secondary-foreground">{qty}</span>
      ...
      <div className="mt-8">
        ...
        <div className="mt-5 bg-card border border-border rounded-xl p-5">
          {activeTab === 'description' && <p className="text-secondary-foreground leading-relaxed">{product.description}</p>}
          {activeTab === 'specs' && (
              ...
              <tbody>
                {Object.entries(product.specs).map(([k, v]) => (
                  <tr key={k} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2.5 text-secondary-foreground">{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {activeTab === 'reviews' && (
            <div className="space-y-4">
              <div className="flex items-center gap-4 pb-4 border-b border-border">
                <div className="text-5xl font-bold font-display text-secondary-foreground">{product.rating}</div>
                <div>
              ...
              <div className="space-y-3 pt-2">
                {[{ name: 'Rakoto A.', rating: 5, comment: 'Excellent product, exactly as described. Very fast shipping!', date: '2026-08-28' }, { name: 'Marie R.', rating: 4, comment: 'Good quality for the price. Would recommend.', date: '2026-08-20' }].map((review, i) => (
                  <div key={i} className="p-3 bg-secondary rounded-xl">
                    ...
                    <p className="text-sm text-secondary-foreground mt-1.5">{review.comment}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'shipping' && (
            <div className="space-y-3 text-sm text-secondary-foreground">
              <div className="flex gap-3 p-3 bg-secondary rounded-lg"><Truck className="w-5 h-5 text-primary flex-shrink-0" /><div><div className="font-medium">Standard delivery — 3-5 business days</div><div className="text-muted-foreground">2,000 - 5,000 Ar</div></div></div>
              <div className="flex gap-3 p-3 bg-secondary rounded-lg"><Zap className="w-5 h-5 text-accent flex-shrink-0" /><div><div className="font-medium">Express delivery — 1-2 business days</div><div className="text-muted-foreground">8,000 - 12,000 Ar</div></div></div>
      ...
      {related.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-bold font-display text-secondary-foreground mb-4">{t("Related products")}</h2>
      ...
      {sellerMore.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-bold font-display text-secondary-foreground mb-4">{t("More from")} {seller.name}</h2>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">

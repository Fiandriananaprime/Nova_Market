  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title={t("Total Revenue")} value="12,580,000 Ar" change={12.4} icon={<DollarSign className="w-5 h-5" />} color="#0077B6" />
        <StatCard title={t("Orders")} value="183" change={8.1} icon={<ShoppingCart className="w-5 h-5" />} color="#5ABCB9" />
        <StatCard title={t("Products")} value="426" change={5.2} icon={<Package className="w-5 h-5" />} color="#0077B6" />
        <StatCard title={t("Low stock items")} value="8" change={-2} icon={<AlertTriangle className="w-5 h-5" />} color="#f59e0b" />
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={revenueData} barSize={20}>
              ...
              <Tooltip formatter={(v) => [(v), 'Revenue']} contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '12px' }} />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary">
                  {[t("Order"), t("Customer"), t("Product"), t("Amount"), t("Status"), t("Date")].map(h => (

...
export default function Inventory() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-xl font-bold font-display text-[var(--foreground)]">Inventory</h1>
      ...
      {lowStock.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <div className="font-medium text-amber-800 dark:text-amber-300 text-sm">{lowStock.length} products need attention</div>
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl overflow-hidden">
        <div className="p-4 border-b border-[var(--border)] flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input placeholder="Search inventory..." className="w-full pl-9 pr-4 py-2 text-sm bg-[var(--secondary)] border border-[var(--border)] rounded-lg text-[var(--foreground)] focus:outline-none focus:border-[#0077B6]" />
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--secondary)]">
                {['Product', 'SKU', 'Current stock', 'Reserved', 'Available', 'Threshold', 'Status'].map(h => (
                ...
                <tr key={item.id} className="hover:bg-[var(--secondary)] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-[var(--foreground)] truncate max-w-36">{item.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-[var(--muted-foreground)]">{item.sku}</td>
                  <td className="px-4 py-3">
                    <span className={`font-bold ${item.stock < item.threshold ? 'text-amber-600' : 'text-[var(--foreground)]'}`}>{item.stock}</span>
                  </td>
                  <td className="px-4 py-3 text-[var(--muted-foreground)]">{item.reserved}</td>
                  <td className="px-4 py-3 text-[var(--foreground)]">{item.available}</td>
                  <td className="px-4 py-3 text-[var(--muted-foreground)]">{item.threshold}</td>

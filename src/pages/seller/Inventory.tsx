...
export default function Inventory() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-xl font-bold font-display text-secondary-foreground">{t("Inventory")}</h1>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="p-4 border-b border-border flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input placeholder={t("Search inventory...")} className="w-full pl-9 pr-4 py-2 text-sm bg-secondary border border-border rounded-lg text-secondary-foreground focus:outline-none focus:border-primary" />
          </div>
                ...
                <tr key={item.id} className="hover:bg-secondary transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-secondary-foreground truncate max-w-36">{item.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{item.sku}</td>
                  <td className="px-4 py-3">
                    <span className={`font-bold ${item.stock < item.threshold ? 'text-amber-600' : 'text-secondary-foreground'}`}>{item.stock}</span>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{item.reserved}</td>
                  <td className="px-4 py-3 text-secondary-foreground">{item.available}</td>

...
export default function SellerReviews() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-xl font-bold font-display text-[var(--foreground)] mb-5">Reviews</h1>

      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5 mb-5 flex items-center gap-8">
        <div className="text-center">
          <div className="text-5xl font-bold font-display text-[var(--foreground)]">{avg.toFixed(1)}</div>
          <Rating value={avg} showCount={false} size="md" />
          <div className="text-sm text-[var(--muted-foreground)] mt-1">{reviews.length} reviews</div>
        </div>
          ...
          {[5, 4, 3, 2, 1].map(r => {
            ...
            return (
              <div key={r} className="flex items-center gap-2 text-sm">
                <span className="w-4 text-[var(--muted-foreground)]">{r}</span>
                ...
                <span className="w-4 text-[var(--muted-foreground)]">{count}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="space-y-4">
        {reviews.map(review => (
          <div key={review.id} className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-medium text-sm text-[var(--foreground)]">{review.customer}</div>
                  <div className="text-xs text-[var(--muted-foreground)]">{review.product}</div>
                </div>
              </div>
              <div className="text-right">
                <Rating value={review.rating} showCount={false} size="xs" />
                <div className="text-xs text-[var(--muted-foreground)] mt-0.5">{review.date}</div>
              </div>
            </div>
            <p className="text-sm text-[var(--foreground)] mb-3">{review.comment}</p>
            ...
            {review.replied ? (
              <div className="ml-4 p-3 bg-[#0077B6]/5 border border-[#0077B6]/20 rounded-xl">
                ...
                <p className="text-sm text-[var(--foreground)]">{review.reply}</p>
              </div>
            ) : (
              replyModal === review.id ? (
                <div className="ml-4 space-y-2">
                  <textarea value={replyText} onChange={e => setReplyText(e.target.value)} rows={2} placeholder="Write your reply..." className="w-full px-3 py-2 text-sm bg-[var(--secondary)] border border-[var(--border)] rounded-xl text-[var(--foreground)] focus:outline-none focus:border-[#0077B6] resize-none" />
                ...
                <button onClick={() => setReplyModal(review.id)} className="flex items-center gap-1.5 text-sm text-[var(--muted-foreground)] hover:text-[#0077B6] transition-colors">
